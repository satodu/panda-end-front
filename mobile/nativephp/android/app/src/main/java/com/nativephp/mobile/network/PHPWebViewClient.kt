...
class PHPWebViewClient(
    ...
    fun handleAssetRequest(url: String, requestHeaders: Map<String, String> = emptyMap()): WebResourceResponse {
        ...
        return try {
            ...
            if (assetFile != null && assetFile.exists()) {
                ...
                val isText = cleanPath.endsWith(".html") || cleanPath.endsWith(".htm") ||
                             cleanPath.endsWith(".css") || cleanPath.endsWith(".js") ||
                             cleanPath.endsWith(".json") || cleanPath.endsWith(".xml") ||
    ...
    fun handlePHPRequest(
        ...
        return try {
            ...
            // Force Content-Type to URL encoded if request is an intercepted staged file upload
            if (postData != null && !postData.startsWith("{") && postData.contains("temp_file_path=")) {
                headers["Content-Type"] = "application/x-www-form-urlencoded"
                Log.d(TAG, "Override Content-Type to application/x-www-form-urlencoded for staged file upload")
            }
            ...
            Log.e(TAG, "❌ Error handling PHP request for path: ${request.url.path}", e)
            errorResponse(500, "Error handling PHP request: ${e.message}")
        }
    }
