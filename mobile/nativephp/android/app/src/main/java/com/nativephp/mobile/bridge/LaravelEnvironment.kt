...
class LaravelEnvironment(private val context: Context) {
    ...
    private fun extractLaravelBundle(): Boolean {
        ...
        // Force extract if the APK itself was updated (packageCodePath lastModified is newer than the extracted .env)
        val apkLastModified = File(context.packageCodePath).lastModified()
        val envFile = File(laravelDir, ENV_FILE)
        val extractedTime = if (envFile.exists()) envFile.lastModified() else 0L

        // If DEBUG mode, ALWAYS extract. Otherwise, only extract if versions don't match
        val isUpToDate = currentVersion?.clean == embeddedVersion.clean && extractedTime >= apkLastModified
        val shouldExtract = embeddedVersion.isDebug || !isUpToDate

        Log.d(TAG, "🔍 DEBUG: isUpToDate = $isUpToDate (extracted=$extractedTime, apk=$apkLastModified)")
