...
class WebViewManager(
    ...
    private fun createWebChromeClient(): WebChromeClient {
        return object : WebChromeClient() {
            ...
            override fun onConsoleMessage(consoleMessage: ConsoleMessage): Boolean {
                Log.d(
                    "$TAG-Console",
                    "${consoleMessage.message()} -- From line ${consoleMessage.lineNumber()} in ${consoleMessage.sourceId()}"
                )
                return true
            }
            ...
            override fun onShowFileChooser(
                ...
                (context as? MainActivity)?.openFileChooser(filePathCallback, fileChooserParams)
                return true
            }
        }
    }
    ...
    private fun createCustomWebViewClient(): WebViewClient {
        return object : WebViewClient() {
            ...
            override fun shouldInterceptRequest(
                ...
                return when {
                    url.contains("/roms/file/") -> {
                        Log.d(TAG, "🎮 Intercepting ROM file stream request directly: $url")
                        serveRomFileDirectly(url)
                    }
        ...
        (function() {
                ...
                if (["post", "patch", "put"].includes(method)) {
                    ...
                    for (var pair of formData.entries()) {
                        ...
                        if (value instanceof File) {
                            hasFile = true;
                            urlEncodedData.append(key, value.name);
                        } else {
                            urlEncodedData.append(key, value);
                        }
                    }
                    if (hasFile) {
                        var tempPath = AndroidPOST.getLastSelectedFilePath();
                        if (tempPath) {
                            urlEncodedData.append('temp_file_path', tempPath);
                        }
                    }
                ...
                if (["post", "patch", "put"].includes(this._method.toLowerCase()) && data) {
                    ...
                    if (data instanceof FormData) {
                        ...
                        data.forEach(function(value, key) {
                            if (value instanceof File) {
                                hasFile = true;
                                urlParams.append(key, value.name);
                            } else {
                        ...
                        if (hasFile) {
                            var tempPath = AndroidPOST.getLastSelectedFilePath();
                            if (tempPath) {
                                urlParams.append('temp_file_path', tempPath);
                            }
                        }
                        bodyStr = urlParams.toString();
                        originalXHRSetHeader.call(this, 'Content-Type', 'application/x-www-form-urlencoded');
                    }
                    AndroidPOST.logPostData(String(bodyStr), this._url, "", reqId);
            ...
            function findAndSendCsrfToken() {
                ...
                if (tokenMeta && tokenMeta.content) {
                    AndroidPOST.storeCsrfToken(tokenMeta.content);
                    return;
                }
    ...
    private fun setupJavaScriptInterfaces() {
        webView.addJavascriptInterface(JSBridge(phpBridge, context, TAG), "AndroidPOST")
    }

    private fun serveRomFileDirectly(url: String): WebResourceResponse {
        ...
        if (metadataFile.exists()) {
            try {
                ...
                val romsJson = JSONObject(jsonStr)
                if (romsJson.has(romId)) {
                    val romObj = romsJson.getJSONObject(romId)
                    val relativePath = romObj.getString("filePath")
                    val romFile = java.io.File(storageDir, "persisted_data/storage/app/private/$relativePath")
                    if (romFile.exists()) {
                        Log.d(TAG, "✅ Serving ROM file directly from: ${romFile.absolutePath} (${romFile.length()} bytes)")
                        val inputStream = java.io.FileInputStream(romFile)
                        val mimeType = "application/octet-stream"
                        val responseHeaders = mutableMapOf<String, String>()
                        responseHeaders["Content-Type"] = mimeType
                        responseHeaders["Content-Length"] = romFile.length().toString()
                        ...
                        Log.e(TAG, "❌ Physical ROM file not found: ${romFile.absolutePath}")
                    }
                } else {
                    Log.e(TAG, "❌ ROM ID not found in metadata: $romId")
                }
            } catch (e: Exception) {
                Log.e(TAG, "❌ Error reading ROM metadata or serving file", e)
            }
        } else {
            Log.e(TAG, "❌ Metadata file not found: ${metadataFile.absolutePath}")
        }
        
        val errorMessage = "{\"message\":\"ROM not found or metadata missing.\"}"
        return WebResourceResponse(
            "application/json",
            "UTF-8",
            404,
            "Not Found",
            mapOf("Content-Type" to "application/json", "Access-Control-Allow-Origin" to "*"),
            java.io.ByteArrayInputStream(errorMessage.toByteArray())
        )
    }
    ...
    fun isStaticAssetExtension(url: String): Boolean {
        val staticExtensions = listOf(
            ...
            ".woff2", ".ttf", ".eot", ".ico", ".json", ".map", ".wasm", ".data",
            ".html", ".htm"
        )
...
class JSBridge(
    ...
    @JavascriptInterface
    fun getLastSelectedFilePath(): String {
        return (context as? MainActivity)?.getLastSelectedFilePath() ?: ""
    }
    @JavascriptInterface
